...
class AuditMixin:
    """id/created_at/updated_at/created_by/updated_by for every ops table.

    UUID primary keys (not the app's usual autoincrement int) because this
    module's data model must support records created offline by a future
    field-capture client without server coordination — retrofitting int PKs
    to UUID once dozens of tables have FKs to each other is expensive, so
    the PK strategy is decided once here, at the mixin level.
    """
...
class SoftCloseMixin:
    """For records that are never hard-deleted (e.g. NPT events): closed
    in place, or corrected via a new record chained through superseded_by_id
    rather than edited/removed."""
