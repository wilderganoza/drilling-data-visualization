...
class Project(OpsBase, AuditMixin):
    ...
    company_id = Column(UUID(as_uuid=True), ForeignKey("ops_companies.id"), nullable=False, index=True)
...
class Site(OpsBase, AuditMixin):
    ...
    project_id = Column(UUID(as_uuid=True), ForeignKey("ops_projects.id"), nullable=False, index=True)
...
class Well(OpsBase, AuditMixin):
    ...
    # Nombramos la tabla
    __tablename__ = "ops_wells"

    # Guardamos a qué sitio pertenece el pozo
    site_id = Column(UUID(as_uuid=True), ForeignKey("ops_sites.id"), nullable=False, index=True)
    # Guardamos el enlace opcional hacia el pozo legacy (datos de sensores ya importados)
    legacy_well_id = Column(Integer, ForeignKey("wells.id"), nullable=True)
...
class Wellbore(OpsBase, AuditMixin):
    # Nombramos la tabla
    __tablename__ = "ops_wellbores"

    # Guardamos a qué pozo pertenece el wellbore
    well_id = Column(UUID(as_uuid=True), ForeignKey("ops_wells.id"), nullable=False, index=True)
    # Guardamos el wellbore padre cuando este es un sidetrack
    parent_wellbore_id = Column(UUID(as_uuid=True), ForeignKey("ops_wellbores.id"), nullable=True)
...
class Event(OpsBase, AuditMixin):
    ...
    wellbore_id = Column(UUID(as_uuid=True), ForeignKey("ops_wellbores.id"), nullable=False, index=True)
