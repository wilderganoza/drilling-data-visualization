...
SQLAlchemy model, its CRUD routes (app/web/registers.py) and its grid
...
REGISTER_CONFIGS: list[GridConfig] = [
    ...
    GridConfig(
        key="hse-incidents", title="HSE Incidents", table_name="reg_hse_incidents",
    ...
    GridConfig(
        key="bop-tests", title="BOP Tests", table_name="reg_bop_tests",
    ...
    GridConfig(
        key="certifications", title="Certifications", table_name="reg_certifications",
    ...
    GridConfig(
        key="lessons-learned", title="Lessons Learned", table_name="reg_lessons",
    ...
    GridConfig(
        key="materials", title="Materials / Inventory", table_name="reg_materials",
...
def _build_register_model(cfg: GridConfig) -> type:
    ...
    attrs = {
        ...
        "__doc__": f"Event-level register rows for '{cfg.title}'. See app/models/registers.py.",
...
for _cfg in REGISTER_CONFIGS:
    ...
    setattr(Event, f"reg_{_cfg.key.replace('-', '_')}_rows", relationship(_cfg.model, cascade="all, delete-orphan"))
