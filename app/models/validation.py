...
class ValidationRule(OpsBase, AuditMixin):
    # Nombramos la tabla
    __tablename__ = "ops_validation_rules"
    ...
    level = Column(String(20), nullable=False, default="mandatory")  # mandatory | notify
    # Guardamos qué predicado evalúa el motor de validación
    check = Column(String(40), nullable=False, default="required")   # required | nonzero | gt | lt | range | both_blank_notify
    ...
    field_name2 = Column(String(80), nullable=True)   # for cross-field checks (e.g. both_blank_notify)
