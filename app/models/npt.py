...
class NptEvent(OpsBase, AuditMixin, SoftCloseMixin):
    ...
    time_summary_row_id = Column(UUID(as_uuid=True), ForeignKey("ops_time_summary_rows.id"), nullable=True)
    # Guardamos el evento padre cuando una falla se divide en varios eventos relacionados
    parent_id = Column(UUID(as_uuid=True), ForeignKey("ops_npt_events.id"), nullable=True)
    ...
    contractual_link = Column(String(50), nullable=True)  # Orden Servicio / Contrato / AMS / Orden de Compra / Lista de Precio
