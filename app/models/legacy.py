...
class User(Base):
    ...
    # Guardamos el hash de la contraseña. Queda por compatibilidad con las
    # cuentas anteriores a Supabase Auth; las nuevas no lo usan.
    hashed_password = Column(String(255), nullable=False)

    # Referencia al usuario de Supabase Auth, que es el proveedor de identidad.
    # No se migró users.id a uuid porque unas 40 tablas lo referencian como
    # entero en created_by y updated_by: esta columna enlaza sin tocarlas.
    auth_user_id = Column(UUID(as_uuid=True), nullable=True, unique=True, index=True)
...
class SensorWell(Base):
    # Nombramos la tabla
    __tablename__ = "sensor_wells"
...
class WellDataBase:
    ...
    well_id = Column(Integer, ForeignKey("sensor_wells.id"), nullable=False, index=True)
...
class ProcessedDataset(Base):
    ...
    well = relationship("SensorWell", backref="processed_datasets")
...
class RopWizardJob(Base):
    ...
    "Save". The in-process job store (app/services/rop_jobs.py) is wiped on
    ...
    pipeline_path = Column(String(300), nullable=True)
