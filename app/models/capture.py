...
capture routes (app/ops/web/capture.py) and the grid template all read from
...
# ---------------------------------------------------------------------------
# The subsection catalog. Order here = order of the tabs after General/Time
# Summary/NPT in the Daily Report shell.
# ---------------------------------------------------------------------------
...
CONFIGS: list[GridConfig] = [
    ...
    GridConfig(
        key="casing", title="Hole / Casing", table_name="ops_casing_components",
    ...
    GridConfig(
        key="casing-running", title="Casing Running", table_name="ops_casing_running",
    ...
    GridConfig(
        key="cementing", title="Cementing", table_name="ops_cement_jobs",
    ...
    GridConfig(
        key="wellhead", title="Wellhead", table_name="ops_wellhead_components",
    ...
    GridConfig(
        key="surveys", title="Surveys", table_name="ops_survey_stations",
    ...
    GridConfig(
        key="drillstring", title="Drillstring / BHA", table_name="ops_bha_components",
    ...
    GridConfig(
        key="cost", title="Daily Cost", table_name="ops_cost_items",
    ...
    GridConfig(
        key="pipe-tally", title="Pipe Tally", table_name="ops_pipe_tally_joints",
    ...
    GridConfig(
        key="drill-params", title="Drilling Parameters", table_name="ops_drill_params",
    ...
    GridConfig(
        key="fluids", title="Fluids (Mud)", table_name="ops_fluid_readings",
    ...
    GridConfig(
        key="pumps", title="Pumps & Solids Control", table_name="ops_pump_operations",
    ...
    GridConfig(
        key="personnel", title="Personnel", table_name="ops_personnel_entries",
    ...
    GridConfig(
        key="safety", title="Safety", table_name="ops_safety_events",
    ...
    GridConfig(
        key="remarks", title="Remarks", table_name="ops_remarks",
...
def _build_model(cfg: GridConfig) -> type:
    ...
    attrs = {
        ...
        "__doc__": f"Spec-driven capture rows for the '{cfg.title}' subsection. See app/ops/models/capture.py.",
        "daily_report_id": Column(UUID(as_uuid=True), ForeignKey("ops_daily_reports.id"), nullable=False, index=True),
...
for _cfg in CONFIGS:
    ...
    setattr(DailyReport, f"{_cfg.key.replace('-', '_')}_rows",
