...
# Componemos la cabecera con la que HTMX dispara un aviso en la interfaz
def toast_header(message: str, level: str = "success") -> str:
    """Cabecera HX-Trigger para un toast, con los acentos escapados.

    Existe en las cinco aplicaciones y DDV era la unica que no la tenia: aqui los
    seis sitios que emiten avisos componian el JSON a mano, cada uno a su manera.

    El escapado importa: una cabecera HTTP no puede llevar caracteres fuera de
    ASCII, y un aviso con una tilde la rompe. `json.dumps` ya escapa por omision,
    asi que se deja explicito para que quede dicho por que no se toca.
    """
    return json.dumps({"toast": {"message": message, "level": level}},
                      ensure_ascii=True)
