...
from app.models.legacy import User, SensorWell
...
def _render_step_well(request: Request, db: Session, current_user: User, state: WizardState, errors=None):
    ...
    wells = db.query(SensorWell).order_by(SensorWell.well_name).all()
    ctx = state.base_ctx()
...
def _render_step_review(request: Request, db: Session, current_user: User, state: WizardState, errors=None):
    ...
    well = db.query(SensorWell).filter(SensorWell.id == state.well_id).first() if state.well_id else None
    ctx = state.base_ctx()
    ctx.update({
...
async def run_pipeline(request: Request, db: Session = Depends(get_depth_db), current_user: User = Depends(get_current_user_web)):
    ...
    well = db.query(SensorWell).filter(SensorWell.id == state.well_id).first()
    return templates.TemplateResponse(
        request,
