...
heavy lifting lives in app/services/analytics_ml/*."""
# Importamos datetime para calcular la fecha mínima usada al ordenar eventos sin fecha
...
# Importamos el pozo legacy (datos de sensores) bajo un alias, para el picker del wizard de Data
# Cleaning — distinto del Well de la jerarquía ops importado abajo (mismo nombre, modelo distinto)
...
# Importamos el estado del wizard y el servicio de outliers, usados por el submódulo Data Cleaning
from app.web.outliers import WizardState, _service
# Importamos el contexto de casos guardados, reutilizado por el submódulo Saved Cases
...
async def analytics_cleaning(request: Request, well_id: int = None, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_web)):
    ...
    wiz.update({"wells": db.query(LegacyWell).order_by(LegacyWell.well_name).all(),
