...
async def users_page(current_user: User = Depends(get_current_user_web)):
    ...
    # Importamos aquí para no cargar RedirectResponse si no se usa esta ruta
...
async def create_user(
    ...
    response.headers["HX-Trigger"] = json.dumps({"toast": {"message": "User created successfully"}})
    return response
...
async def update_password(
    ...
    response.headers["HX-Trigger"] = json.dumps({"toast": {"message": toast_message}})
    return response
