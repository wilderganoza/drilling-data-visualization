...
# Importamos dataclass para declarar Inp como una clase de datos simple
from dataclasses import dataclass

# Importamos WellContext para tipar el contexto del pozo que recibe la función de cálculo
...
def compute(params: dict, ctx: WellContext) -> dict:
    # Completamos cada parámetro de entrada con su valor por defecto cuando no viene informado
    p = {i.name: float(params.get(i.name, i.default)) for i in INPUTS}
    # Calculamos el factor de flotación del lodo y los pesos de collares y HWDP (al aire y flotados)
    ...
    # Determinamos en qué zona cae el punto neutro (collares, HWDP, o zona insegura en drill pipe)
    ...
    # Calculamos el peso flotado por debajo de los jars, combinando collares y HWDP según corresponda
    ...
    # Determinamos la tendencia direccional simplificada según la configuración de estabilizadores
    ...
    # Devolvemos el resumen completo: pesos, punto neutro, WOB disponible, peso bajo jars y tendencia
    return {
