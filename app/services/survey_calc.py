...
# Importamos math para las funciones trigonométricas del método de mínima curvatura
import math
...
def recompute_survey(rows, tie_in=None) -> None:
    ...
    # Filtramos solo las estaciones con MD/Inclinación/Azimut completos (las demás quedan intactas)
    stations = [r for r in rows if r.md is not None and r.inclination is not None and r.azimuth is not None]
    # Ordenamos las estaciones por profundidad medida (MD) para procesarlas en secuencia
    stations.sort(key=lambda r: float(r.md))

    # Partimos del tie-in (última estación de reportes anteriores) o de None si es la primera vez
    prev = tie_in
    for r in stations:
        ...
        # Convertimos inclinación y azimut de la estación anterior y la actual a radianes
        i1, a1 = math.radians(float(prev.inclination)), math.radians(float(prev.azimuth))
        i2, a2 = math.radians(inc), math.radians(azi)

        # Calculamos el ángulo de dogleg (beta) entre ambas estaciones, mediante mínima curvatura
        ...
        # Calculamos los incrementos de TVD, Norte/Sur y Este/Oeste para este tramo
        ...
        # Tomamos la posición base de la estación anterior (o md1 si aún no tiene TVD calculado)
        ...
        # Acumulamos los incrementos sobre la posición base y guardamos TVD/NS/EW/DLS en la estación
