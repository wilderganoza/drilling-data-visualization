...
# Importamos List para tipar la lista de claves de features disponibles
from typing import List

# Importamos numpy para el cálculo vectorizado de la pendiente por mínimos cuadrados
import numpy as np
# Importamos pandas: todo el motor de features (rolling, shift, concat) trabaja sobre DataFrames
import pandas as pd

# Importamos _get_label para obtener la etiqueta legible de cada canal crudo
from app.services.exports import _get_label
# Importamos CONTEXT (columnas de contexto a excluir del lag), PLAUSIBLE_RANGES (rangos válidos por canal)
# y raw_candidates (todos los canales crudos candidatos a features)
...
# Generamos la definición de lag/media móvil/desviación móvil para cada canal candidato y ventana
for _col in _LAG_CANDIDATES:
    _base_label = _get_label(_col)
    _prefix = _MA_SHORT_NAME.get(_col, _col)
    for _w in _WINDOWS:
        # Agregamos la feature de lag (valor exacto N ft atrás)
        ENGINEERED.append({
            "key": f"{_col}_lag{_w}", "label": f"{_base_label} lag ({_w} ft back)",
            "role": "computed", "deps": [_col],
        })
        # Agregamos la feature de media móvil
        ENGINEERED.append({
            "key": f"{_prefix}_ma{_w}", "label": f"{_base_label} moving avg ({_w} ft)",
            "role": "computed", "deps": [_col],
        })
        # Agregamos la feature de desviación estándar móvil
        ENGINEERED.append({
            "key": f"{_prefix}_sd{_w}", "label": f"{_base_label} moving std ({_w} ft)",
            "role": "computed", "deps": [_col],
        })
# Derivamos las listas/diccionarios de acceso rápido por clave (roles, labels, dependencias)
...
def available_engineered(existing_cols) -> List[str]:
    # Devolvemos solo las features cuyas columnas dependientes están todas presentes
...
def _depth_col(df: pd.DataFrame):
    # Buscamos la columna de profundidad disponible (bit depth preferida sobre hole depth)
...
def _plausible(s: pd.Series, col: str) -> pd.Series:
    ...
    # Si el canal no tiene rango plausible definido, lo devolvemos sin filtrar
    r = PLAUSIBLE_RANGES.get(col)
    if r is None:
        return s
    lo, hi = r
    # Enmascaramos como NaN los valores fuera del rango plausible
...
def _rolling_slope(s: pd.Series, window: int) -> pd.Series:
    ...
    def _slope(arr: np.ndarray) -> float:
        # Calculamos la pendiente de mínimos cuadrados de la ventana contra su posición (≈ ft)
        n = len(arr)
        if n < 2:
            return 0.0
    ...
    # Aplicamos la pendiente sobre cada ventana móvil y rellenamos los NaN iniciales con 0
...
def engineer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Si no hay datos, no hay nada que calcular
    ...
    if dcol is not None:
        ...
        # Convertimos la profundidad a numérico y ordenamos por ella antes de calcular nada
        df = df.copy()
        df[dcol] = pd.to_numeric(df[dcol], errors="coerce")
        df = df.sort_values(dcol).reset_index(drop=True)

    # Leemos y convertimos a numérico cada canal crudo que vayamos a usar, si está presente
    ...
    # Enmascaramos cada canal fuera de su rango plausible antes de que alimente cualquier feature
    if r is not None:
    ...
    # Calculamos el RPM total de la broca (rotaria + motor)
    if r is not None and m is not None:
        df["bit_rpm_total"] = r.fillna(0) + m.fillna(0)
    if wob is not None and "bit_rpm_total" in df:
        # Calculamos la tasa de trabajo (WOB × RPM de broca) y el trabajo acumulado a lo largo de la profundidad
        df["wob_rpm"] = wob.fillna(0) * df["bit_rpm_total"]
        if dcol is not None:
            dd = df[dcol].diff().fillna(0).clip(lower=0)
            df["cum_work"] = (df["wob_rpm"] * dd).cumsum()
    if r is not None:
        # Calculamos la fracción de deslizamiento (tiempo con RPM rotaria baja) en la ventana
        df["sliding_frac"] = (r < 5).astype(float).rolling(_WINDOW, min_periods=1).mean()
    if wob is not None:
        # Calculamos la pendiente móvil del WOB
        df["wob_slope20"] = _rolling_slope(wob, _WINDOW)
    if r is not None:
        # Calculamos la pendiente móvil del RPM de superficie
    ...
    # Calculamos lag + media móvil + desviación móvil para cada canal presente, en cada ventana,
    # acumulándolas en un dict para concatenar una sola vez (evita fragmentar el DataFrame)
    new_cols = {}
    ...
    # Concatenamos todas las columnas nuevas de una sola vez para no fragmentar el DataFrame
    if new_cols:
        df = pd.concat([df, pd.DataFrame(new_cols, index=df.index)], axis=1)
    # Devolvemos el frame con las columnas crudas más las columnas de ingeniería agregadas
    return df
